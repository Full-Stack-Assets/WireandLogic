...
<FAQ>
  ...
  <Question q="How can I secure a webhook endpoint?">Use HTTPS, validate signatures or secret tokens provided by the sender, restrict IP ranges if possible, and store secrets securely.</Question>
  <Question q="What fallback strategies work if a webhook fails?">Implement retry queues with exponential back‑off, log failed deliveries, and optionally run a low‑frequency poll as a safety net to catch missed events.</Question>
</FAQ>
