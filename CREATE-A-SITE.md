...
- **Engine (optional)** — `llm` (the writer's OpenAI-compatible endpoint, model,
  and which env var holds its key — defaults to **Groq**; any OpenAI-compatible
  provider such as OpenRouter works too) and `imageProvider` (`'pexels'`
...
- **The LLM key** — required. Its name must match `llm.apiKeyEnv` in
  `site.config.ts` — **`GROQ_API_KEY` by default** (`OPENROUTER_API_KEY` or
  another provider's key if you switch). Groq's free tier comfortably covers
  one post per hour.
