...
// (15-min CPU budget) that imports the same `runPipeline`. On supported managed hosts, the
// default serverless function budget is enough on Pro; set `maxDuration` if
// you see timeouts.
export const runtime = 'nodejs';
export const maxDuration = 300; // Maximum route runtime: 5 minutes
...
function isAuthorized(req: Request): boolean {
  ...
  // Scheduled callers send `Authorization: Bearer <CRON_SECRET>`
